...
class XTTrader:
    ...
    def execute_trade(self, direction: str, order_type: str = "MARKET",
        ...
        qty, size_mode, size_reason = self.risk.calculate_position_size(
            symbol, price, requested_leverage, sl_price, order_type)
        if qty <= 0:
        ...
        while not self._stop_monitor.is_set():
            ...
            try:
                ...
                if now - last_scan >= scan_interval:
                    last_scan = now
                    result = self.scan_and_execute()
                    if result.get("action") == "signal":
                        outcome = self.execute_trade(
                            result["direction"],
                            self._decide_order_type(result["result"]),
                            self._decide_time_in_force(result["result"]),
                        )
                        logger.info(f"Auto-trade result: {outcome}")
                        # Notify user if the trade was rejected after the signal was sent.
